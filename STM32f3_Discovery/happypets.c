...
#define puertoMotores GPIOD //y Leds
#define puertoComunicaciones GPIOD // GPIOC
#define puertoBotones GPIOD//GPIOA
...
int comidaBaja = 10;
int aguaBaja = 11;
int comandoDispensarComida = 12;
int comandoDispensarAgua = 13;
...
static void gpio_setup(void) {
	  	rcc_peripheral_enable_clock(&RCC_AHBENR, RCC_AHBENR_IOPEEN);//dar reloj a puerto E escribir valor correspondiente en registro RCC_AHBENR
//	  	rcc_peripheral_enable_clock(&RCC_AHBENR, RCC_AHBENR_IOPDEN);//encender puerto D
//		rcc_peripheral_enable_clock(&RCC_AHBENR, RCC_AHBENR_IOPAEN);//encender puerto A
//		rcc_peripheral_enable_clock(&RCC_AHBENR, RCC_AHBENR_IOPCEN);//encender puerto C

		rcc_periph_clock_enable(RCC_GPIOD);//habilitar reloj para puerto D

		  	gpio_mode_setup(GPIOE, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO8| GPIO9| GPIO10| GPIO11| GPIO12| GPIO13| GPIO14| GPIO15);//leds STM32
		  	gpio_mode_setup(GPIOD, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLDOWN, motorComida| motorAgua);//motores

		  	gpio_mode_setup(GPIOD, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, ledAgua| ledComida | ledConfiguracion);//leds

		  	gpio_mode_setup(GPIOD, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, botonComida | botonAgua | botonAceptar);//botones

		  	gpio_mode_setup(GPIOD, GPIO_MODE_INPUT, GPIO_PUPD_PULLDOWN, comidaBaja | aguaBaja | comandoDispensarComida | comandoDispensarAgua );//comunicaciones

//		  	gpio_mode_setup(GPIOA, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO8| GPIO9| GPIO10| GPIO11| GPIO12| GPIO13| GPIO14| GPIO15);
//		  	gpio_mode_setup(GPIOC, GPIO_MODE_INPUT, GPIO_PUPD_NONE, GPIO8| GPIO9| GPIO10| GPIO11| GPIO12| GPIO13| GPIO14| GPIO15);
...
//    rcc_periph_clock_enable(RCC_GPIOD);//habilitar para puerto D
//    rcc_periph_clock_enable(RCC_GPIOC);//habilitar para puerto C
    ...
    gpio_set_af(GPIOE, GPIO_AF2, GPIO9);//ser controlado por interrupts?
